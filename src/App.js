...
function reducer(state,action){
  switch(action.type){
    ...
    case("start"):
      return{...state,status:"active",secsRemaining: state.questions.length *30} 
    case("tick"):
      return{   ...state,
...
function App() {
  const[{status,questions,index,answer,points,secsRemaining},dispatch]= useReducer(reducer,initialState)
  ...
  return (
    <div className="app">
      ...
      <Main>
        ...
        {status==="active" &&
        <>
         <Progress index={index} numQuestions={numQuestions} answer={answer} points={points} maxPoints={maxPoints}/>
